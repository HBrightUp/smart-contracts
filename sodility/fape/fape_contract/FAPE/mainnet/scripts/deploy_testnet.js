...
const ISSUE_GAME_AWARD = "0x858DCCFF98873d81222874bBc56E062432217f35";

async function deployAllContract() {

  /*
  //deploy TokenUSDT
  const usdtFactory = await hre.ethers.getContractFactory("TokenUSDT");
  const usdt = await usdtFactory.deploy();
  await usdt.deployed();
  console.log("TokenUSDT address:", usdt.address);
*/
...
//deploy  TicketNFT
  ...
  //gover init
  trx = await gover.connect(owner).setTokenALC(tokenFAPE_address);
  console.log("goverment setTokenALC hash: ", trx.hash);

  trx = await gover.connect(owner).setTicketNFT(ticketNFT_address);
  console.log("goverment setTicketNFT hash: ", trx.hash);

  /*
  trx = await gover.connect(owner).setDapp(dapp);
  console.log("goverment setDapp hash: ", trx.hash);
*/
  trx = await gover.connect(owner).setUSDT(tokenUSDT_address);
  console.log("goverment setUSDT hash: ", trx.hash);
/*
  trx = await gover.connect(owner).setGameAwardAddr(ISSUE_GAME_AWARD);
  console.log("goverment setGameAwardAddr hash: ", trx.hash);

  //need manul
  //gover.setALCGiftAmount
  trx = await gover.connect(owner).setALCGiftAmount(new ethers.BigNumber.from("1000000000000000000000"));
  console.log("goverment setALCGiftAmount hash: ", trx.hash);

  //gover.setAdmissionTicket
  trx = await gover.connect(owner).setAdmissionTicket(new ethers.BigNumber.from("200000000000000000000"));
  console.log("goverment setAdmissionTicket hash: ", trx.hash);
*/
  //ticketNFT init
