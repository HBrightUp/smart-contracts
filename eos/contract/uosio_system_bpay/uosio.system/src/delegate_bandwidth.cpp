  ...
  /**
   ...
   void validate_b1_vesting( int64_t stake ) {
	...
	// **********<begin>This code was modified by camphor 2018-12-10 </begin>*********//
	   bool system_contract::prodbw_verf(name producer){
	      del_bandwidth_table     del_tbl( _self, producer.value);
	      //Find the amount that delegate to self
	      auto itr = del_tbl.get( producer.value,"prodbw verfy was not pass in del_tbl");
	      if(itr.cpu_weight + itr.net_weight >= asset(PRODUCE_MIN_BW, symbol{"UOS", 4})) {
	         return true;
	      } else {
	         return false;
	      }
	   ...
	   void system_contract::penaltyprodbw( name producer){
	      require_auth(_self);
	      del_bandwidth_table     del_tbl( _self, producer.value);
	      //Find the amount that delegate to self
	      auto itr = del_tbl.get( producer.value,"prodbw verfy was not pass in del_tbl");

	      uosio_assert( itr.cpu_weight + itr.net_weight >= asset(PRODUCE_MIN_BW,symbol{"UOS", 4}),"producer delegate is too small, error?");
	      if(itr.cpu_weight >= asset(PRODUCE_MIN_BW,symbol{"UOS", 4})){
	            changebw(producer, producer, asset(0, symbol{"UOS", 4}),-asset(PRODUCE_MIN_BW,symbol{"UOS", 4}), false, true);
	      }else{
	            changebw(producer, producer, -asset(PRODUCE_MIN_BW,symbol{"UOS", 4}) + itr.cpu_weight,-itr.cpu_weight , false, true);
	      }
                                   ...
                                   const asset stake_net_delta, const asset stake_cpu_delta, bool transfer, bool penaltyprod  )
   {
      ...
      // update stake delegated from "from" to "receiver"
      {
         ...
         // producer delegate amount in the active state cannot be too low
         {
            ...
            if(  ((stake_net_delta.amount + stake_cpu_delta.amount) < 0) && (from == receiver) ){
               auto prod = _producers.find( from.value );
	  ...
	  // **********<begin>This code was modified by camphor 2018-12-10 </begin>*********//
       if(penaltyprod){
           cancelallvote(from, true);
             auto t_data = make_tuple( stake_account, bvpay_account,-stake_net_delta - stake_cpu_delta,std::string("pb penaltyprod"));
             action(permission_level{stake_account, active_permission}, token_account,
                    "transfer"_n, t_data)
                     .send();
       }
       // **********<end>This code was modified by camphor 2018-12-10 </end>*********//
      ...
      if ( (stake_account != source_stake_from) && (!penaltyprod )) { //for uosio both transfer and refund make no sense
	  ...
	  //Begin: remvoed by hml Date:2019-11-12
      //vote_stake_updater( from );
	  //End: remvoed by hml Date:2019-11-12
   ...
   void system_contract::update_voting_power( const name& voter, const asset& total_update )
   {
      ...
      if( voter_itr->vote_producers.size() > 0 ) {
            //update_votes( voter, voter_itr->proxy, voter_itr->producers, false );
	      validate_votes( voter, total_update);
      }
