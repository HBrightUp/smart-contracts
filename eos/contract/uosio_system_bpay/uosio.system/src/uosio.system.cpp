...
namespace uosiosystem {
   ...
   :native(s,code,ds),
	...
	_sys_args_list(_self, _self.value),
	_uosc_bp_list(_self, ("uosclist"_n).value),
   _bpvtpay( ("uosio.bvpay"_n), ("uosio.bvpay"_n).value),
   _bvrealpay(("uosio.bvpay"_n), ("uosio.bvpay"_n).value)
   {
   ...
   void system_contract::updtrevision( uint8_t revision ) {
      ...
      //End: removed by hml Date:2019-11-20
#endif
   ...
   void system_contract::init( unsigned_int version, symbol core ) {
      ...
      INLINE_ACTION_SENDER(uosio::token, open)( token_account, { _self, active_permission },
