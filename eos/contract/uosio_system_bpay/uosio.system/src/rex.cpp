...
#include <uosio.system/rex.results.hpp>
...
namespace uosiosystem {
   ...
   void system_contract::buyrex( const name& from, const asset& amount )
   {
      ...
      rex_results::buyresult_action buyrex_act( rex_account, std::vector<uosio::permission_level>{ } );
   ...
   void system_contract::sellrex( const name& from, const asset& rex )
   {
      ...
      if ( current_order.success ) {
         rex_results::sellresult_action sellrex_act( rex_account, std::vector<uosio::permission_level>{ } );
   ...
   void system_contract::check_voting_requirement( const name& owner, const char* error_msg )const
   {
      ...
      //Begin: modify by hml Date:2019-11-18
      check( vitr != _voters.end() && ( vitr->proxy || 15 <= vitr->vote_producers.size() ), error_msg );
       //End: modify by hml Date:2019-11-18
   #endif
   }
   ...
   void system_contract::runrex( uint16_t max )
   {
      ...
      /// transfer from uosio.names to uosio.rex
         ...
         for ( uint16_t i = 0; i < max; ++i ) {
            ...
            if ( bitr != _rexbalance.end() ) { // should always be true
               ...
               if ( result.success ) {
                  ...
                  rex_results::orderresult_action order_act( rex_account, std::vector<uosio::permission_level>{ } );
   ...
   int64_t system_contract::rent_rex( T& table, const name& from, const name& receiver, const asset& payment, const asset& fund )
   {
      ...
      table.emplace( from, [&]( auto& c ) {
         ...
         c.expiration   = current_time_point() + uosio::days(30);
         c.loan_num     = pool->loan_num;
      });

      rex_results::rentresult_action rentresult_act{ rex_account, std::vector<uosio::permission_level>{ } };
   ...
   void system_contract::channel_to_rex( const name& from, const asset& amount )
   {
      ...
      if ( rex_available() ) {
         ...
         transfer_act.send( from, rex_account, amount,
                            std::string("transfer from ") + from.to_string() + " to uosio.rex" );
      }
#endif
   }
   ...
   #else 
      static const time_point_sec rms{0};
   #endif
      return rms;
   }
   ...
   #if FUNCTION_SWITCH_REX
      /**
       * If CORE_SYMBOL is (UOS,4), maximum supply is 10^10 tokens (10 billion tokens), i.e., maximum amount
