...
namespace uosiosystem {
    ...
    *
    */
   void system_contract::regproducer( const name producer, const uosio::public_key& producer_key, const std::string& url, uint16_t location ) {
   ...
   void system_contract::update_votes( const name voter_name, const name proxy, const std::vector<name>& producers, bool voting ) {
      //validate input
      if ( proxy ) {
         uosio_assert( producers.size() == 0, "cannot vote for producers and proxy at same time" );
         uosio_assert( voter_name != proxy, "cannot proxy to self" );
      } else {
         uosio_assert( producers.size() <= 30, "attempt to vote for too many producers" );
      ...
      uosio_assert( !proxy || !voter->is_proxy, "account registered as a proxy is not allowed to use a proxy" );
      ...
      if ( voter->last_vote_weight > 0 ) {
         if( voter->proxy ) {
            ...
            uosio_assert( old_proxy != _voters.end(), "old proxy not found" ); //data corruption
      ...
      if( proxy ) {
         ...
         uosio_assert( new_proxy != _voters.end(), "invalid proxy specified" ); //if ( !voting ) { data corruption } else { wrong vote }
         uosio_assert( !voting || new_proxy->is_proxy, "proxy not found" );
   ...
   void system_contract::propagate_weight_change( const voter_info& voter ) {
      uosio_assert( !voter.proxy || !voter.is_proxy, "account registered as a proxy is not allowed to use a proxy" );
