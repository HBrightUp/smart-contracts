#include <uosio.msig/uosio.msig.hpp>
#include <uosiolib/action.hpp>
#include <uosiolib/permission.hpp>
                        ...
                        ignore<transaction> trx )
{
   ...
   uosio_assert( _trx_header.expiration >= uosio::time_point_sec(current_time_point()), "transaction expired" );
   //uosio_assert( trx_header.actions.size() > 0, "transaction must have at least one action" );

   proposals proptable( _self, _proposer.value );
   uosio_assert( proptable.find( _proposal_name.value ) == proptable.end(), "proposal with the same name exists" );
   ...
   uosio_assert( res > 0, "transaction authorization failed" );
...
void multisig::approve( name proposer, name proposal_name, permission_level level,
                        const uosio::binary_extension<uosio::checksum256>& proposal_hash )
{
   require_auth( level );
   ...
   if ( apps_it != apptable.end() ) {
      ...
      uosio_assert( itr != apps_it->requested_approvals.end(), "approval is not on the list of requested approvals" );
      ...
      uosio_assert( itr != apps.requested_approvals.end(), "approval is not on the list of requested approvals" );
...
void multisig::unapprove( name proposer, name proposal_name, permission_level level ) {
   ...
   if ( apps_it != apptable.end() ) {
      ...
      uosio_assert( itr != apps_it->provided_approvals.end(), "no approval previously granted" );
      apptable.modify( apps_it, proposer, [&]( auto& a ) {
      ...
      uosio_assert( itr != apps.provided_approvals.end(), "no approval previously granted" );
      old_apptable.modify( apps, proposer, [&]( auto& a ) {
...
void multisig::cancel( name proposer, name proposal_name, name canceler ) {
   ...
   if( canceler != proposer ) {
      uosio_assert( unpack<transaction_header>( prop.packed_transaction ).expiration < uosio::time_point_sec(current_time_point()), "cannot cancel until expiration" );
   }
   ...
   if ( apps_it != apptable.end() ) {
      ...
      uosio_assert( apps_it != old_apptable.end(), "proposal not found" );
...
void multisig::exec( name proposer, name proposal_name, name executer ) {
   ...
   ds >> trx_header;
   uosio_assert( trx_header.expiration >= uosio::time_point_sec(current_time_point()), "transaction expired" );
...
UOSIO_DISPATCH( uosio::multisig, (propose)(approve)(unapprove)(cancel)(exec)(invalidate) )
